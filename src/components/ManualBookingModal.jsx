...
import { format, addDays, startOfMonth, endOfMonth, eachDayOfInterval, getDay, isSameDay, isAfter, startOfToday, parse, addMinutes, isBefore, startOfDay, areIntervalsOverlapping, isToday } from 'date-fns';
import { it } from 'date-fns/locale';

const ManualBookingModal = ({ isOpen, onClose, event, isInline = false }) => {
    ...
    const availableSlots = useMemo(() => {
        ...
        rules.forEach(rule => {
            ...
            // Valid slot condition: slot start + duration must be before or equal to rule end
            while (isBefore(addMinutes(current, duration), end) || format(addMinutes(current, duration), 'HH:mm') === format(end, 'HH:mm')) {
                const slotEnd = addMinutes(current, duration);

                // 1. Skip if slot is in the past (only if selected date is today)
    ...
    const content = (
        <motion.div
            initial={isInline ? { opacity: 0, height: 0 } : { opacity: 0, scale: 0.95, y: 20 }}
            animate={isInline ? { opacity: 1, height: 'auto' } : { opacity: 1, scale: 1, y: 0 }}
            exit={isInline ? { opacity: 0, height: 0 } : { opacity: 0, scale: 0.95, y: 20 }}
            className={`card w-full ${isInline ? 'max-w-none shadow-none border-primary/20 bg-primary/5 mt-4' : 'max-w-4xl relative z-10 shadow-2xl bg-[#111] border-white/10'} p-0 overflow-hidden flex flex-col`}
        ...
        >
            ...
            <div className="p-6 border-b border-white/5 flex justify-between items-center bg-white/5">
                <div>
                    ...
                    <p className="text-xs text-text-muted italic">Stai prenotando per: <span className="text-primary font-bold">{event?.title}</span></p>
                </div>
                ...
                <AnimatePresence mode="wait">
                    {step === 1 && (
                        <motion.div key="step1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="text-center text-xs font-bold mb-2 capitalize">{format(currentMonth, 'MMMM yyyy', { locale: it })}</div>
                            ...
                            <div className="flex flex-col">
                                            ...
                                            <div className="grid grid-cols-2 gap-2">
                                                {availableSlots.map(slot => (
                                                    <button
                                                        ...
                                                        className={`w-full py-2 px-3 rounded-xl border-2 transition-all text-xs font-bold text-center ${selectedSlot === slot
                                                            ? ''
                                ...
                                {selectedSlot && (
                                    <button onClick={() => setStep(2)} className="btn btn-primary w-full mt-4 py-3 text-xs font-bold shadow-lg shadow-primary/20">Prosegui</button>
                                )}
                            </div>
                        </motion.div>
                    )}
                    ...
                    {step === 2 && (
                        <motion.div key="step2" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="max-w-lg mx-auto py-4">
                            <button onClick={() => setStep(1)} className="text-primary text-xs font-bold flex items-center gap-1 mb-6 hover:translate-x-[-4px] transition-transform"><ChevronLeft size={16} /> Indietro</button>
                            ...
                            <form onSubmit={handleBooking} className="space-y-4">
                                ...
                                <button type="submit" disabled={submitting} className="btn btn-primary w-full py-4 mt-4 font-bold shadow-xl shadow-primary/20">
